...
export default class OkxWallet extends IWallet {
  ...
  async connect() {
    ...
    if (typeof window.okxwallet.bitcoin !== 'undefined') {
      let btcAddr = await window.okxwallet.bitcoin.requestAccounts();
  ...
  buildTx = async (rawTx, address) => {
    ...
    let signedMessage = await commons.omnilock.bitcoin.signMessage(tx.signingEntries.get(0).message, "ecdsa", window.okxwallet.bitcoin);
