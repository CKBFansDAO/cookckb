...
import { encodeToAddress, parseAddress, TransactionSkeleton, createTransactionFromSkeleton } from "@ckb-lumos/helpers";
...
export default class MetaMaskWallet extends IWallet {
    ...
    // 初始化MetaMask监听器和获取账户信息
    initialize() {
        //console.log('initialize')
        if (provider) {
            if (providerDetails.find(item => item.info.rdns === 'io.metamask')) {
                provider = providerDetails.find(item => item.info.rdns === 'io.metamask').provider;
            }
            // 监听账户变化
    ...
    async signRawTx(rawTx, address) {
        ...
        throw new Error('Please change the MetaMask account first');
    }
}
